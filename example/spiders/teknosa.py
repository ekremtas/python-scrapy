...
class Example2Spider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for href in response.css('div.product-name a::attr(href)').extract():
    ...
    def parse_page(self, response):
        ...
        product_allcomment = response.css(
            '.button-more-results::attr(href)').extract()
        ...
        product_numbercomment = response.css(
            '#comments-container').css('::text').extract()
        # product_price
